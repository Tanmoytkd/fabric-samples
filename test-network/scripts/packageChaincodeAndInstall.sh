...
packageChaincode() {
  set -x
  peer lifecycle chaincode package ${CC_NAME}-$ORG.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label ${CC_NAME}_${CC_VERSION} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
installChaincode() {
  setGlobals $ORG
  set -x
  peer lifecycle chaincode install ${CC_NAME}-$ORG.tar.gz >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
## Install chaincode
infoln "Install chaincode on peer0.org$ORG..."
installChaincode
